# game.py
# Gesamte Spiel-Logik & Daten in EINEM Modul
import os
import time
...
# Optional: mehrere Speicherstände
try:
...
SAVES_FOLDER = "saves"  # Fallback-Ordner für Spielstände
...
# ------------------------------ #
# Fallback: Speicherstände finden #
# ------------------------------ #
...
def hauptmenue_loop():
    ...
    while True:
        print("=== Willkommen zu deinem RPG ===")
...
def ausruhen(player):
    ...
    for _ in range(secs):
        ...
        time.sleep(1)
        player.heal(1)
        print(f"+1 HP ({player.health}/{player.max_health()})")
...
# ---------------- #
def start_game():
    """Startet das Spiel (Titelmenü → Spielschleife)."""
    while True:
        clear()
